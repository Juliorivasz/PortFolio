...
function Projects({ theme }: ProjectsProps) {
    ...
    function mostrarDescripcion(e: React.MouseEvent<HTMLDivElement>) {
        const dataType = e.currentTarget.getAttribute('datatype');
      
        if (dataType) {
          const index = parseInt(dataType, 10);
          const descripcion = document.querySelectorAll(`.description`) as NodeListOf<HTMLElement>;
      
          if (descripcion[index] instanceof HTMLElement) {
            descripcion[index].style.transform = 'translateY(-100%)';
            descripcion[index].style.opacity = '1';
            descripcion[index].style.overflow = 'overlay';
          }
        }
      }
      
      
      function cerrarDescripcion(e: React.MouseEvent<HTMLDivElement>) {
        const dataType = e.currentTarget.getAttribute('datatype');
        ...
        if (dataType) {
          ...
          if (descripcion[index] instanceof HTMLElement) {
            descripcion[index].style.transform = 'translateY(0)';
            descripcion[index].style.opacity = '0';
            descripcion[index].style.transition = 'all 1s';
          }
        }
      }
